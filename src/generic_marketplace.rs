    ...
    impl GenericMarketplace {
        pub fn start_marketplace(
            ...
            let marketplace_listing_key =
                ...
                .metadata(metadata! {
                    init {
                    "name" => "Marketplace Key".to_string(), updatable;
        ...
        pub fn set_marketplace_fee(&mut self, new_fee: Decimal, proof: Proof) {
            proof.check(self.marketplace_admin);

            self.marketplace_fee = new_fee;
        }

        pub fn set_mint_fee(&mut self, new_fee: Decimal, proof: Proof) {
            proof.check(self.marketplace_admin);
        ...
        pub fn multi_listing_purchase(
            ...
            for (address, order_group) in grouped_orders {
                ...
                if order_group.len() == 1 {
                    ...
                    // Handle fee
                    let is_there_fee = result.1.pop().is_some();
